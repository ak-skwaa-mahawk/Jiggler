python -c '
with open("pwc/walker.py", "w") as f:
    f.write("""# pwc/walker.py
from .types import *
...
class Walker:
    def __init__(self):
        self.current_holonomy_mode: str = "normal"
    ...
    def compile_schedule(
        ...
        relaxation_base = 1.0
        pressure_scale = 1.0
        self.current_holonomy_mode = "normal"

        if drift_error > 0.06:
            self.current_holonomy_mode = "conservative"
            relaxation_base = 0.75
            pressure_scale = 0.92
        elif stability > 0.98:
            self.current_holonomy_mode = "refinement"
            relaxation_base = 1.12

        for action in priority_actions:
            if action.get("action") == "curvature_regularization":
        ...
        for t in range(plan.horizon_steps):
            ...
            steps.append(ControlStep(
                tick_index=t,
                pid_target_drift=spin_target,
                pressure_cap_global=pressure_target,
                relaxation_strength_global=relaxation,
                regional_overrides=[],
            ))
    ...
    def tick(self, engine: SubstrateEngine, step: ControlStep) -> SubstrateSnapshot:
        engine.set_setpoints(
            spin=step.pid_target_drift,
            pressure=step.pressure_cap_global,
            temp=0.0,
        )
    ...
    def get_status(self) -> Dict:
        return {"holonomy_mode": self.current_holonomy_mode}
""")
print("✅ pwc/walker.py updated to match current ControlStep")
'
python -c '
with open("pwc/walker.py", "w") as f:
    f.write("""# pwc/walker.py
...
class Walker:
    ...
    def compile_schedule(
        ...
        for t in range(plan.horizon_steps):
            ...
            # Use the new ManifoldPlan structure
            spin_target = (1 - α) * gs.spin + α * plan.drift_envelope.preferred_drift
            pressure_target = (1 - α) * gs.pressure + α * (plan.pressure_budget.global_budget * pressure_scale)
            temp_target = (1 - α) * gs.temp + α * 0.0
    ...
    def get_status(self) -> Dict:
        return {"holonomy_mode": self.current_holonomy_mode}
""")
print("✅ pwc/walker.py updated successfully")
'
