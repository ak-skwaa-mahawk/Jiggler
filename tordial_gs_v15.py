...
# FULL CONFIG LOAD
# =============================================================================
# Fallback configuration dict if file missing
try:
...
CURV_COUPLING = {
    ("A", "B"): float(t.get("coupling_A_to_B", 0.1)),
    ("B", "A"): float(t.get("coupling_B_to_A", 0.1)),
    ("A", "C"): float(t.get("coupling_A_to_C", 0.1)),
    ("C", "A"): float(t.get("coupling_C_to_A", 0.1)),
    ("B", "C"): float(t.get("coupling_B_to_C", 0.1)),
    ("C", "B"): float(t.get("coupling_C_to_B", 0.1)),
}
        ...
        while i < len(unpaired):
            ...
            for j in range(i + 1, len(unpaired)):
                if unpaired[i][1] == unpaired[j][1]:
                    ...
                    del unpaired[j]; del unpaired[i]; matched = True; break
            if not matched: i += 1
        return corrections

class RingGovernor:
    def __init__(self, target_sigma: float = 120.0, kp: float = 0.012, ki: float = 0.003, kd: float = 0.006):
        self.target = target_sigma; self.kp = kp; self.ki = ki; self.kd = kd
...
class TordialAgentNode:
    ...
    OMEGA_RADS: float = 2.0 * math.pi * 60.0
    CHASE_RATIO_TAU: float = TAU_3D / 1.65036
    def __init__(self, d: int, r: int, node_id: int):
        self.node_id = node_id; self.d = d; self.r = r; self.load = 0.0
        self.decision = "STABLE"; self.load_history: List[float] = []
...
class OpenTordialAgentNode(TordialAgentNode):
    def __init__(self, d: int, r: int, node_id: int, x: float = 0.0, y: float = 0.0):
        ...
        self.sigma_T: float = 0.0; self.fission_count: int = 0; self.quarantined: bool = False; self.high_kappa_streak: int = 0
        self._denom = 4.0 * PHI_OP * GEAR_SHIFT
    def compute_and_update_gs(self, curvature_pressure: float, resonance: float) -> float:
        if curvature_pressure > 0.6:
            self.d += max(1, int(resonance * 0.35 + self.drift_phase * 0.1))
            if resonance > 0.4 and random.random() < 0.45: self.r += 1
    ...
    def measure_syndrome(self) -> List[str]:
        s: List[str] = []
        if self.sigma_T < -180: s.append("e")
        if abs(self.drift_phase % (math.pi / 2)) > 0.38: s.append("m")
        return s
    def apply_governor_correction(self, delta_d: int, delta_r: int) -> None:
        self.d = max(4, self.d + delta_d); self.r = max(20, self.r + delta_r)
...
def persist_node_state(node: OpenTordialAgentNode, ring: str = "A", parent_id: Optional[int] = None):
    ...
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute("INSERT INTO nodes (node_id, ring, d, r, sigma_T, drift_phase, fission_count, parent_id) VALUES (?,?,?,?,?,?,?,?)",
...
class CurvatureField:
    ...
    def compute(self, avg_sigma: float, avg_kappa: float, global_energy: float, logical_error_rate: float, node_count: int) -> Tuple[float, float]:
        k_norm = max(0.0, min(1.0, avg_kappa / 12.0)); e_norm = max(0.0, min(1.0, global_energy / 3000.0))
        s_norm = max(-1.0, min(1.0, avg_sigma / 500.0)); ler = max(0.0, min(1.0, logical_error_rate))
        bp = 0.45 * k_norm + 0.35 * e_norm + 0.20 * max(0.0, s_norm)
        pressure = max(0.0, min(1.25, bp * (1.0 - 0.6 * ler) + max(0.0, min(0.3, (node_count - 24) / 80.0))))
        resonance = max(0.0, min(1.0, (0.55 * k_norm + 0.35 * max(0.0, s_norm) + 0.10 * e_norm) * (1.0 - 0.8 * ler)))
        pressure = 0.35 * pressure + 0.65 * self.last_pressure
        resonance = 0.35 * resonance + 0.65 * self.last_resonance
        self.last_pressure = pressure; self.last_resonance = resonance
        return pressure, resonance

class TripleRingTordialMatrix:
    def __init__(self, node_count: int = 12):
        self.current_tick = 0; self.decoder = SurfaceCodeDecoder()
    ...
    def _seed_rings(self, node_count: int):
        ...
        print(f"[+] Triple-ring geometry seeded natively: {node_count} nodes mapped.")

    def _visualize_torus(self):
        pass # Clean headless fallback
    ...
    def execute_heavy_load_cycle(self, system_load: float = 1.0):
        ...
        for r_name, nodes in [("A", self.nodes_a), ("B", self.nodes_b), ("C", self.nodes_c)]:
            ...
            p, r = f_obj.compute(avg_sigma, avg_kappa, self.energy[r_name], self.ler[r_name], len(nodes))
            for n in nodes:
                n.compute_and_update_gs(p * system_load, r)
            gov = getattr(self, f"governor_{r_name.lower()}")
            dd, dr = gov.step(avg_sigma)
            for n in nodes: n.apply_governor_correction(dd, dr)
...
if __name__ == "__main__":
    ...
    print("\n[+] Direct local simulation test verified clean execution.")
EOF
