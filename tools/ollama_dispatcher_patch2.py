...
OLLAMA_TOOLS = [
    {
        ...
        "function": {
            ...
            "description": "Runs a 19-channel QEEG and Cardiac HRV analysis snapshot and returns Z-scores and Observer Epsilon coherence.",
            "parameters": {
                "type": "object",
                "properties": {
                    "theta_spike_intensity": {"type": "number", "description": "Simulated theta spike intensity"}
                },
                "required": []
            }
        }
    },
    {
        ...
        "function": {
            ...
            "description": "Injects quadrature displacements (delta_q, delta_p) into a GKP bosonic code state and computes stabilizer shift corrections.",
            "parameters": {
                "type": "object",
                "properties": {
                    "delta_q": {"type": "number", "description": "Position shift error delta_q"},
                    "delta_p": {"type": "number", "description": "Momentum shift error delta_p"},
                    "squeezing_delta": {"type": "number", "description": "Finite squeezing parameter Delta (default 0.1)"}
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Computes cross-spectral coherence, phase angle lag, and time delay between two target EEG channels.",
            "parameters": {
                "type": "object",
                "properties": {
                    "ch1": {"type": "string", "description": "First channel name (e.g. F3)"},
                    "ch2": {"type": "string", "description": "Second channel name (e.g. F4)"},
                    "band_low": {"type": "number", "description": "Frequency low bound"},
                    "band_high": {"type": "number", "description": "Frequency high bound"}
                },
                "required": ["ch1", "ch2"]
            }
        }
    },
    {
        ...
        "function": {
            ...
            "description": "Applies MasterKey Newton-Raphson deflation to recover canonical ground state from an inflated surplus claim.",
            "parameters": {
                "type": "object",
                "properties": {
                    "surplus_y": {"type": "number", "description": "Inflated surplus state Y"},
                    "prime_modes": {"type": "array", "items": {"type": "integer"}, "description": "Prime modes stack [3, 5, 7]"}
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Executes hybrid post-quantum key exchange (X25519 + PQC Lattice KEM).",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Generates custom binaural beat or isochronic tone WAV file at a given target frequency.",
            "parameters": {
                "type": "object",
                "properties": {
                    "mode": {"type": "string", "enum": ["binaural", "isochronic"]},
                    "target_freq": {"type": "number", "description": "Target frequency in Hz"},
                    "carrier_freq": {"type": "number", "description": "Carrier frequency in Hz"}
                },
...
def execute_tool_call(tool_name: str, tool_args: dict) -> dict:
    if tool_name == "run_gkp_stabilizer_check":
        dq = float(tool_args["delta_q"])
        dp = float(tool_args["delta_p"])
        sq = float(tool_args.get("squeezing_delta", 0.1))

        gkp = GKPSqueezedEngine(delta=sq)
        res = gkp.measure_syndrome_and_correct(dq, dp)

        return {
            "injected_noise": {"delta_q": dq, "delta_p": dp},
            "stabilizer_syndromes": {"syndrome_q": float(res["syndrome_q"]), "syndrome_p": float(res["syndrome_p"])},
            "reverse_correction": {"correct_q": float(res["correction_q"]), "correct_p": float(res["correction_p"])},
            "residual_drift": {"q": float(res["residual_q"]), "p": float(res["residual_p"])},
            "state_recovered": res["within_threshold"],
            "max_threshold": float(res["error_threshold_limit"])
        }
